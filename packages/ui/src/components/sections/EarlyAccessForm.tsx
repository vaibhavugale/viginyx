      ...
      <div className="max-w-5xl mx-auto flex flex-col items-center">
        ...
        <div className="text-center space-y-6 mb-16 max-w-4xl">
          <h2 className="reveal-item text-4xl md:text-5xl lg:text-7xl font-bold tracking-tight text-brand-1 leading-[1.1]">
            We're Building This <span className="text-brand-8">For You</span> —
            Get Early Access
          </h2>

          <p className="reveal-item text-base md:text-lg text-brand-4 max-w-3xl mx-auto leading-relaxed">
            Viginyx is onboarding its first cohort of{" "}
            <span className="text-brand-8 font-semibold">
              NABH-accredited and NABH-aspiring hospitals
            </span>{" "}
            in Kalyan-Dombivli for a{" "}
            <span className="text-brand-1 font-bold">free 4–8 week pilot</span>.
            No commitment. No payment. First-come, first-served.
          </p>
            ...
            {[
              {
                ...
                text: "Free Pilot — No Cost",
              },
              {
                icon: <MapPin className="w-3.5 h-3.5" />,
                text: "KDMC Hospitals Prioritised",
              },
              {
                icon: <Clock className="w-3.5 h-3.5" />,
                text: "4–8 Week Programme",
              },
            ].map((pill, i) => (
              <div
                key={i}
                className="flex items-center gap-2 px-4 py-2 bg-brand-9/50 border border-brand-5/10 rounded-full text-[11px] font-bold text-brand-3"
              >
        ...
        <div className="reveal-item w-full bg-[#F8F9FA] border border-black/[0.04] rounded-[40px] md:rounded-[48px] p-8 md:p-12 lg:p-16 relative overflow-hidden shadow-sm">
          ...
          <form className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10">
            ...
            <div className="space-y-4">
              <label className="block text-[10px] font-black text-brand-5 uppercase tracking-widest">
                Full Name *
              </label>
              <input
                type="text"
            ...
            <p className="md:col-span-2 text-center text-xs md:text-sm text-brand-4 font-medium leading-relaxed">
              We'll reach out within 48 hours. Pilot is free — no commitment
              required.{" "}
              <span className="text-brand-8">
                KDMC hospitals prioritised in Round 1.
              </span>
            </p>
          </form>
        </div>
