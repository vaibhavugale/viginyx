    ...
    () => {
      ...
      simTl
        ...
        .to(".sim-node-0", {
          y: -8,
          scale: 1.02,
          boxShadow: "0 20px 40px -10px rgba(0,181,184,0.15)",
          duration: 0.8,
        })
        .to(
          ".sim-icon-0",
        ...
        .to(
          ".sim-node-0",
          { y: 0, scale: 1, clearProps: "boxShadow", duration: 0.8 },
          "-=0.3",
        )
          ...
          <div className="flex flex-col md:flex-row gap-8 lg:gap-12 relative z-10">
            {nodes.map((node, i) => (
              <div key={i} className="flex-1 node-card">
                ...
                <div
                  className={`sim-node-${i} bg-white rounded-3xl p-6 md:p-8 border shadow-sm transition-all duration-300 relative z-10 h-full flex flex-col ${node.containerClass}`}
                >
                  <div className="flex items-center gap-3 mb-6">
                    <div
                      className={`sim-icon-${i} w-10 h-10 rounded-xl flex items-center justify-center border ${node.iconBg} shrink-0 transition-colors`}
                    >
                      {node.icon}
                    </div>
                  ...
                  <h3 className="text-base font-bold text-brand-1 mb-3">
                    {node.title}
                  </h3>
