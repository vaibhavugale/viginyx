    ...
    >
      ...
      >
        ...
        <div className="relative z-10 max-w-4xl flex flex-col items-center px-4 pt-16 pb-12">
          <div
            ref={badgeRef}
            className="flex items-center gap-3 px-5 py-2.5 bg-white rounded-full subtle-shadow border border-brand-5/10 mb-6 hover:scale-105 transition-transform cursor-default"
          >
            <div className="w-5 h-5 rounded-full bg-primary flex items-center justify-center text-white text-[9px]">
              ●
            </div>
            <span className="text-xs font-bold text-brand-1">
              New! Blockchain ADR audit trails for NABH
            </span>
          </div>
            ...
            <span ref={titleLine1Ref} className="block">
              ...
              <span className="text-primary inline-block">
                {"Pharmacovigilance".split("").map((char, i) => (
                  <span
                    key={i}
                    className="pv-letter inline-block origin-bottom"
                  >
                    {char}
                  </span>
                ))}
          ...
          <p
            ref={subtextRef}
            className="text-sm md:text-lg lg:text-xl text-brand-3 max-w-2xl mx-auto mb-8 leading-relaxed font-medium"
          >
            Automate ADR reporting and causality assessments in under 60
            seconds. Focus on patient safety, while we handle the compliance.
          </p>
