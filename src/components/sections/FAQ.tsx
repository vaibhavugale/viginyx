      ...
      <div className="max-w-7xl mx-auto bg-[#F8F9FA] border border-black/[0.04] rounded-[48px] md:rounded-[64px] relative overflow-hidden p-8 md:p-16 lg:p-20 shadow-sm">
        ...
        <div className="reveal-header text-center max-w-3xl mx-auto mb-16 space-y-6 relative z-10">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-white rounded-full shadow-sm border border-brand-5/10 mx-auto">
            ...
            <span className="text-[10px] font-bold text-brand-1 uppercase tracking-widest">Support Knowledge</span>
          </div>
          ...
          {faqs.map((faq, i) => {
            ...
            return (
              ...
              >
                ...
                >
                  <h4 className={`text-base md:text-lg font-bold transition-colors duration-300 ${isOpen ? "text-brand-8" : "text-brand-1 group-hover:text-brand-8"}`}>
                    {faq.q}
                  </h4>
                  <div className={`shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 ${isOpen ? "bg-brand-8/10 text-brand-8 rotate-180" : "bg-brand-9 text-brand-5"}`}>
