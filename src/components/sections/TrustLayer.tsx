...
export default function TrustLayer() {
  ...
  useGSAP(() => {
    ...
    tl.fromTo(".highlight-letter", 
        { y: 40, opacity: 0, rotateX: -90 },
        { y: 0, opacity: 1, rotateX: 0, duration: 0.8, stagger: 0.03, ease: "back.out(2)" }
    )
    .fromTo(".normal-word", 
        { y: 20, opacity: 0 },
    ...
    .fromTo(".check-circle",
        { scale: 0, opacity: 0 },
        { scale: 1, opacity: 1, duration: 0.4, stagger: 0.5, ease: "back.out(2)" },
        "-=0.4"
    )
    .fromTo(".check-text",
        { clipPath: "inset(0 100% 0 0)" },
        { clipPath: "inset(0 0% 0 0)", duration: 0.6, stagger: 0.5, ease: "none" },
        "<"
    );
    ...
    return (
        <span className="text-primary inline-block perspective-[1000px]">
            {word.split("").map((char, i) => (
                <span key={i} className="highlight-letter inline-block origin-bottom">{char}</span>
            ))}
        </span>
    );
  };
      ...
      <div className="max-w-7xl mx-auto bg-brand-9/30 rounded-[48px] md:rounded-[64px] relative overflow-hidden p-8 md:p-16 lg:p-20">
        ...
        <div className="max-w-3xl mx-auto mb-16 space-y-6 relative z-10 text-center flex flex-col items-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-white border border-brand-5/10 rounded-full shadow-sm">
                ...
                <span className="text-[10px] font-bold text-brand-1 uppercase tracking-widest">The Trust Layer</span>
            </div>
            ...
            >
                {renderHighlightedWord("Immutable")} <span className="normal-word inline-block">Records.</span> <br />
                <span className="normal-word inline-block">Unbreakable</span> {renderHighlightedWord("Trust.")}
            </h2>

            <p className="normal-word text-lg md:text-xl text-brand-3 leading-relaxed font-medium max-w-2xl mt-4">
                Every ADR assessment, form submission, and data modification is cryptographically sealed on a blockchain-backed audit trail. Compliant with DPDP Act 2023 data privacy mandates and NABH 6th Edition requirements—ensuring your institution is always audit-ready.
            </p>
        </div>
                ...
                <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
                    ...
                    <div className="h-14 bg-white border-b border-black/5 flex items-center justify-between px-4 md:px-6 shrink-0 relative z-10 shadow-[0_1px_5px_rgb(0,0,0,0.01)]">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-sm font-bold text-brand-1">Trust Layer Validation Matrix</span>
                        </div>
                        <div className="flex items-center gap-4 hidden sm:flex">
                             <div className="text-[11px] font-bold tracking-wide text-brand-5 bg-brand-9 px-2.5 py-1 rounded-md">Last updated: Just now</div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 h-full">
                            ...
                            <div className="bg-white rounded-[16px] border border-black/5 p-5 shadow-sm flex flex-col hover:shadow-md transition-shadow relative overflow-hidden">
                                <div className="flex items-center gap-2 mb-5">
                                    ...
                                    <div>
                                        <h4 className="text-[12px] font-bold text-brand-1 uppercase tracking-wider">Report Verification</h4>
                                        <div className="text-[9px] font-medium text-brand-5 uppercase tracking-wider">CDSCO Format</div>
                                    </div>
                                </div>
                                ...
                                <div className="flex-1 bg-white rounded-xl border border-black/5 p-5 relative shadow-[0_2px_10px_rgba(0,0,0,0.02)] flex flex-col">
                                    ...
                                    <div className="space-y-3 mb-5">
                                        {[
                                            ...
                                            <div key={i} className="flex items-center gap-2.5">
                                                ...
                                                <div className="check-text text-[10px] font-bold text-brand-1 tracking-wide whitespace-nowrap">{check}</div>
                                            </div>
                                        ))}
                                    </div>
                                    ...
                                    <div className="absolute bottom-3 right-3 z-10 pointer-events-none">
                                        <div className="bg-white/95 backdrop-blur-sm px-4 py-3 rounded-2xl shadow-xl border border-emerald-200 flex items-center gap-3 transform rotate-[-4deg] scale-100">
                                            ...
                                            <div>
                                                <div className="text-emerald-600 font-black tracking-widest uppercase text-sm leading-tight drop-shadow-sm">VERIFIED</div>
                                                <div className="text-[8px] font-bold text-brand-5 tracking-wider font-mono mt-0.5">HASH MATCHED</div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="bg-white rounded-[16px] border border-black/5 p-5 shadow-sm flex flex-col hover:shadow-md transition-shadow">
                                <div className="flex items-center justify-between mb-5">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div>
                                            <h4 className="text-[12px] font-bold text-brand-1 uppercase tracking-wider">Live Audit Trail</h4>
                                            <div className="text-[9px] font-medium text-brand-5 uppercase tracking-wider">Blockchain Ledger</div>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-1.5 px-2 py-1 bg-emerald-50 rounded-md border border-emerald-100">
                                        <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" />
                                        <span className="text-[9px] font-bold text-emerald-600 tracking-wider">SYNCING</span>
                                    </div>
                                </div>
                                
                                <div className="space-y-3 flex-1 flex flex-col justify-center">
                                    {[
                                        { hash: "0x8F92A1B2...3912", type: "ADR Submission", status: "Verified", time: "2 min ago" },
                                        { hash: "0x3C419F0E...843A", type: "Data Modification", status: "Verified", time: "15 min ago" },
                                        { hash: "0x7A2B4D5C...112B", type: "Consent Granted", status: "Verified", time: "1 hr ago" },
                                        { hash: "0x1E8DB3A4...99F2", type: "System Event", status: "Verified", time: "3 hrs ago" }
                                    ].map((log, i) => (
                                        <div key={i} className="p-3 bg-[#F8FAFC] rounded-[10px] border border-black/[0.03] space-y-2 relative group hover:border-purple-500/30 transition-all cursor-default">
                                            <div className="flex justify-between items-start">
                                                <div className="text-[11px] font-mono text-brand-1 font-bold group-hover:text-purple-600 transition-colors">{log.hash}</div>
                                                <span className="text-[9px] text-brand-5 font-medium">{log.time}</span>
                                            </div>
                                            <div className="flex flex-wrap items-center gap-2">
                                                <span className="text-[9px] px-2 py-0.5 bg-purple-50 text-purple-600 rounded-md font-bold tracking-wider">{log.type}</span>
